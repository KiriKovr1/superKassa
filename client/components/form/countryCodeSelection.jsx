...
import { getCodes, updateCountryCode, updateSelectionVisiability} from '../../redux/reducer/input'
...
const CountryCodeSelection = () => {
    ...
    return (
        <div className='countryCodeSelection'>
            ...
            {codes.data ? codes.data.map((countryCode, index) => {
                return (
                    <CountryCode country={countryCode.country} code={countryCode.code} key={`countryCode${index}`}/>
                )
            }) : <Loader /> }
        </div> 
    )
